...
- **Let someone join a kitchen by tapping a link, not typing a code** – four questions, listed at the end of the spec. Design for the `/j/` page is one of them.
  <sub>Next – v1.1</sub>
...
- [x] **🎉 1.1.0 IS LIVE ON THE APP STORE, RELEASED 2026-08-20 ~21:10 CEST.**
      Thomas: *"It's thursday evening here (21:09) please release"* – approved
      by Apple that morning, released the same evening.

      **The state, read back from Apple's API rather than the dashboard:**
      1.1.0 `READY_FOR_SALE`, carrying **build 25**; 1.0 stays `READY_FOR_SALE`
      on build 12 as the superseded record. Before the release it was
      `PENDING_DEVELOPER_RELEASE`, review submission
      `b3cdc162-943c-4b6b-b36d-8bbafa37c07d` `COMPLETE`, submitted
      `2026-08-19T20:32Z`.

      ⚠️ **`READY_FOR_SALE` is Apple's word for "released", not "on every
      phone".** Propagation across storefronts takes hours, and an install
      already on a phone updates on the App Store's own schedule unless the user
      pulls it. So the three-week gap does not close tonight – it closes as
      people update.

      **Released with the API, not the button:**
      `node scripts/asc-release-version.mjs --release --version 1.1.0`, written
      for this release and kept. Dry run by default, refuses anything not in
      `PENDING_DEVELOPER_RELEASE`, and re-reads the version afterwards rather
      than trusting Apple's `201` – the same lesson as the build state script.

      **⏱ Under a day, against thirteen for 1.0.** Submitted Wednesday evening,
      approved by Thursday morning. Worth writing down because the 1.0 wait
      shaped every release decision since – "second reviews are quick" was the
      argument for a 1.0.1 back on 2026-08-13, and it turns out to have been
      right. It does not make the next one fast; one sample is one sample.

      **What this puts in front of real users, who had been on build 12 since
      2026-07-30:** everything in builds 13–25 – the Settings tab, sharing with
      30-day expiry and Stop sharing, the leftover moves, the ingredient
      sections. Three weeks of finished work, all of it landing at once.

      **What ships with it that is known and imperfect:** the Danish sharing
      strings are Claude's unproof-read drafts (flagged three times before
      submitting; correcting them now costs a version, not a build), and the
      expired-share screen has still never been seen on a phone because no link
      is old enough. Neither is a reason to hold the release – both were decided
      before it was submitted.

      **1.2.0 is unblocked now:** App Store Connect would not let a 1.2.0
      version record exist while 1.1.0 was in review. Build 26 (1.2.0,
      drag-to-reorder) is already VALID on TestFlight, so the record can be
      created whenever the version is ready to be prepared.
...
- [ ] **Let someone join a kitchen by tapping a link, not typing a code**
      Needs: Thomas – four questions, listed at the end of the spec. Design for
           the `/j/` page is one of them.
      📄 **THE SPEC IS THE ENTRY: [join-by-link.md](join-by-link.md)** (sketched
           2026-08-20, not built). Read it rather than the research below, which
           is kept for the record.
      ⚠️ **THE SPEC ADDS ONE THING THE RESEARCH DID NOT REACH: a code in a URL
           is not the same object as a code on a fridge.** `PREP-XXXX` is ~615k
           combinations, which is safe today only because a code DISCLOSES
           NOTHING until it is redeemed by a signed-in, throttled user. A web
           page that names the household turns it into an unauthenticated lookup
           key, and 615k is nothing at web scale - an afternoon of requests
           would harvest codes to family names. Same shape as the `anon` listing
           bug caught in the share spec. **Proposal: the link carries its own
           unguessable token, the friendly code stays for reading aloud, and
           both share one expiry.**
...
- [x] **Point the App Store listing at prepeat.app instead of the GitHub URL – DONE**
      Verified against App Store Connect 2026-08-19, all three:
           privacy `https://prepeat.app/privacy.html`, support
           `https://prepeat.app/support.html`, marketing `https://prepeat.app`.
           On the live 1.0 record AND on 1.1.0, so it is not waiting on a release.
      Nothing was ever broken - the github.io URLs redirected - so this was
           always about the listing looking like Thomas's own domain rather than
           a borrowed one.
...
- [x] **⭐ Share a recipe with someone – SHIPPED, in 1.1.0 (submitted 2026-08-19)**
      Built, deployed and submitted. `share.prepeat.app` serves the page,
           migrations 0034/0035/0038 are on production, and build 25 carries the
           app half. All three locked decisions held: a **teaser** not the whole
           recipe, an **imported photo is never published**, and **Save to my
           recipes on an explicit tap**.
      Thomas's case for it, worth keeping now that it exists: *"without 'mouth to
           mouth' sharing, this app will not be a success. And having a recipe as
           a carrier will be key."* Whether that is true is now measurable.
      📄 Specs: [share-recipe.md](share-recipe.md) and
           [share-expiry-and-stop-sharing.md](share-expiry-and-stop-sharing.md).
      ⚠️ **The design asks this item was waiting on are NOT done** – desktop
           frames for the share site, a 404 frame, and the ⋯ ruling. They are
           carried out to their own item below rather than closed with the
...
- [ ] **⚠️ The Danish for the sharing strings shipped unproof-read**
      ...
      **It is in the submitted binary.** Four strings written by Claude and
           never checked: `recipes.detail.stopSharing` ("Stop deling"),
           `stopSharingBody`, `share.expiredTitle` ("Linket er udløbet") and
           `share.expiredBody`. Flagged three times before submitting and not
           acted on, which was a fair call - but a correction now costs a
           version, not a build.
      The wider proof-read is still outstanding too: the spreadsheet Claude sent
           on 2026-08-18, 281 unique strings, duplicates removed, 11 flagged, no
           placeholder mismatches.
      Where: `src/locales/da.ts`, and the copy table in
           [share-expiry-and-stop-sharing.md](share-expiry-and-stop-sharing.md).
...
- [ ] **⭐ Household tags on a recipe – "protein rich", "dessert" – and tap one to filter**
      ...
      **Which is why the heart does not survive this item** (decision 3): the
      ...
      4. **Tags do NOT travel to another household.** *"No – they belong to the
         ...
         stays inside one household's lineage, so tags survive THAT copy.
      5. **Anybody in the household can edit or delete a tag.** *"anybody in
         the household can edit/delete tags."* No roles, no owner. Deleting a
      ...
      - The add-meal sheet ALREADY has the identical search field and
        `All` / `Favorites` chip row, and already calls the same
        `matchesSearch()` ([add-meal-sheet.tsx:409](../src/components/plan/add-meal-sheet.tsx)).
        So tags land in both places for very nearly the price of one.
      - **The picker is where the filter earns most.** Browsing is when you are
        curious; planning is when you have a real question – "something quick
        for Tuesday", "a dessert for Saturday". It is also where AND pays off
        (quick AND veggie) and where zero results are likeliest, which is the
        same reason the empty state above has to be designed.
...
- [x] **Update the keep-awake frames to the shipped copy – DONE 2026-08-20**
      Three recipe frames now match the app, styles and fonts untouched:
           709:6815 and 709:6822 "Screen on while cooking" → **"Screen stays
           on"**, and 709:6858 "Screen d**i**mes while cooking" → **"Screen
           dims"**. The typo went with the string, so the old "fix the dimes
           typo" item is resolved rather than still pending.
      ⚠️ **A SEARCH FOR THE COPY FOUND TWO MORE, ON A DIFFERENT SCREEN, AND
           THEY WERE LEFT ALONE ON PURPOSE.** They are the SETTINGS row, not the
           recipe status line - a different string Thomas ruled on separately:
      - **684:3871 "Screen stays on"** – what the app's Settings actually ships;
           `en.ts` cites this node as the source. Already correct.
      - **709:7587 "Screen on while cooking"** – the older version of that row,
           the one Thomas rewrote after Claude's first attempt ran to three
           lines. Now the only place in the file still carrying the old copy.
      Left because a superseded frame may be kept deliberately as a "before",
           and deleting someone's reference is not a tidy-up to make unasked.
           Raised with Thomas instead.
      **The lesson worth keeping:** the backlog named two node ids; the file had
           five matches across two pages, two of them a different string
           entirely. Searching the document beat trusting the note.
...
- [x] **DONE 2026-08-17. The share host is live at share.prepeat.app**
      Deployed, domained, certificate issued, and proven end to end against
      PRODUCTION data. Deployment Protection confirmed off by fetching
      anonymously – the check Thomas cannot run from his own signed-in browser.
      **The live page immediately found a bug local testing had missed:** a
      recipe with no prep time rendered "Total 30 min  Cook 30 min", the same
      number twice, because both local test recipes happened to have prep AND
      cook set. Fixed, and all four combinations checked this time.
      **Left open deliberately:** no Git integration, so deploys are manual
      (`npx vercel deploy --prod` from `~/Documents/Claude/Projects/prepeat-share`,
      which is linked). Worth connecting a repo before anyone else touches it.
      Old text follows.
      Needs: nothing.
      Why: the host is BUILT and PROVEN – deployed anonymously on 2026-08-17 and
           verified against PRODUCTION: a real share of a real recipe rendered
           with the sharer's name and, being imported, no description; unknown
           and malformed tokens 404; revoking took the page down 36 seconds
           later. But the Vercel token available to Claude can read the team and
           **cannot create a project** (`403 forbidden`), and the temporary
           deployment expires in 60 minutes.
      Steps: (1) create a `prepeat-share` project on Vercel, or grant
           project-creation rights; (2) point `share.prepeat.app` at it;
           (3) then the ungate item below.
      Where: `~/Documents/Claude/Projects/prepeat-share` (local git, no remote).
           Its README carries the deploy notes.

      (2026-08-17.)

- [x] **DONE 2026-08-18. The share preview card is generated, and it was the
      thing making sharing not work.** Without `og:image` iMessage builds no card
      at all – a grey bubble reading `share.prepeat.app`. Proved by sending two
      links from the same server differing only in having an image, then fixed:
      every page now carries one, generated at `/og/<token>.png` when the
      recipe's own photo is not ours to publish. Confirmed in Messages.
      **Filed as polish on 2026-08-17, which was wrong** – it was the
      precondition for the feature working at all.

- [x] **⭐ FOR THE MORNING: publish the privacy policy, then submit 1.1.0 – DONE 2026-08-19**
      1. ✅ **Privacy policy published** and live at prepeat.app/privacy.html,
           dated 19 August. **The weak wording never went out.** This item was
           written when the prepared section said a link could only be turned off
           by writing to us; because it was never published, the section went
           live once, already describing 30-day expiry and Stop sharing.
      2. ✅ **1.1.0 submitted** – WAITING_FOR_REVIEW at 20:32 UTC with **build
           25**, not the build 24 named below. Build 24 predated share expiry and
           Stop sharing; the record was still pointing at it hours after build 25
           was VALID on TestFlight, because **uploading a build does not attach
           it to the version record**.
      3. ❌ **The Danish was NOT proof-read** – carried out to its own item
           below, because it is no longer a review before shipping. It shipped.
...
- [x] **⭐ Share links: 30-day expiry + "Stop sharing" in the menu**
      ...
      Why: writing the privacy policy exposed that a user cannot turn a share
           link off. The policy had to say "write to us", which is weak as a
           GDPR withdrawal route, and it is the gap Thomas was most worried
           about.
      Decided 2026-08-18: **30 days, absolute from creation** (48h breaks a link
           sent Friday and opened Monday; 90 was Claude's and leaned on expiry
           carrying the whole burden, which Stop sharing removes).
           **Stop sharing revokes EVERY link for that recipe** – the only honest
...
- [x] **The submitted "What's New" now mentions expiry and Stop sharing – fixed 2026-08-19**
      The release notes attached to 1.1.0 were written for **build 24** and never
           revised: accurate about sharing, Danish, the screen staying on,
           imports and settings, and **silent on the two things build 25 added**.
           Users would have been told they could share a recipe and not told that
           a link lapses after 30 days or that they could turn it off – while the
           privacy policy, published the same day, said both.
      Fixed by adding one section after the sharing one, leading with the control
           rather than the expiry. Read back from Apple: 1411 characters.
      ⚠️ **THE EDIT DID NOT COST THE PLACE IN THE QUEUE.** The worry was that
           changing metadata would bounce the submission back. It did not: same
           submission id, still WAITING_FOR_REVIEW, checked immediately after the
           write. Worth knowing for next time – **metadata is editable while
           WAITING_FOR_REVIEW**, and only frozen once it is IN_REVIEW.
      ⚠️ **THE LESSON IS THE CHECK, NOT THE FIX.** Nothing would have caught this
           except reading the live What's New from Apple. It had been correct
           when written, and went stale because a build was added after it. **Read
           the What's New back before submitting, every release** – it is set
           once and silently outlives the build it describes.

- [x] **⭐ Ship share expiry + Stop sharing as part of 1.1.0 – LIVE 2026-08-20**
      🎉 **Shipped.** Apple approved 1.1.0 (build 25) on the morning of
           2026-08-20, less than a day after it was submitted at 20:32 UTC on
           2026-08-19, and Thomas released it the same evening. Sharing with
           30-day expiry and Stop sharing is now in real users' hands, not just
           TestFlight's.
      Everything it needed, all live: migration 0038 on production (verified in
           the schema dump, not just the ledger), the share site deployed
           (expired page + the AASA split), the privacy policy published and
           dated 19 August.
      ⚠️ **THE POLICY WENT OUT ONCE, ALREADY CORRECT.** The draft written before
           the feature said a link could only be turned off by writing to us –
           weak as a GDPR withdrawal route, and the thing Thomas was most worried
           about. It was never published, so it never had to be.
      ⚠️ **THE DANISH WAS NEVER PROOF-READ.** `stopSharing`, `stopSharingBody`,
           `expiredTitle` and `expiredBody` are Claude's drafts and they are in
           the submitted binary. Flagged three times before submitting and not
           acted on, which is a fine call to make – but it means a correction now
           costs a version, not a build. Check them on the translation sheet.
      ⚠️ **THE EXPIRED SCREEN HAS STILL NEVER BEEN SEEN BY ANYONE ON A PHONE.**
           No link is old enough. The web version was verified against the
           deployed host; the in-app one has been read but not run, and it is
           the one with no Figma frame behind it. First real sighting will be
           30 days after the first share link of this release.
...
- [x] **DONE 2026-08-18. The chat card is Thomas's design and it is LIVE.**
      ...
      **The generator did go**, as anticipated below: no token lookup, no
      ...
      ✅ **THE DESIGN LANDED 2026-08-18** – Figma "The chat card" on page
           ...
           white, the "+" in `text/link` (#56C91D), right-aligned with 168px of
           padding. **No title, no times, no sender.**
      Why: the card shipped 2026-08-18 is Claude's mocha design, chosen as an
           interim "till my design lands". It is the thing every recipient
           decides from, so it is worth drawing properly.
      Where: `api/og/[token].js` in prepeat-share – the layout only. Fetching,
           fonts, type-size stepping, caching and the safe area stay.
      ⚠️ **THE DESIGN CARRIES NOTHING PER-RECIPE, SO THE GENERATOR CAN GO.**
           Every card is byte-identical, so `/og/&lt;token&gt;.png` can become
           one static PNG in `public/` – no `@vercel/og`, no fonts fetched at
           the edge, no per-request render, and the token stops leaking into an
           image URL. The measured constraints below stop mattering for the same
           reason. **Raise before deleting the generator**, since restoring it
           is work if a title is ever wanted on the card.
      The constraints that shaped the interim card, kept in case a title returns:
           titles run 6&ndash;72 characters (median 19), the card renders about
           236px wide in a chat so 26px of caption is 7px on screen, and some
           apps crop to a square, which takes ~285px off each side.
      When it lands: an owned recipe WITH a photo uses that photo; everything
...
- [x] **⚠️ UNGATE "Share recipe" when the share page ships – DONE 2026-08-18**
      The `IS_DEV_APP` gate is gone from `menuItems` and the header comment in
           `recipe-shares.ts` no longer says the page does not exist. Verified in
           the code, not from memory: the share item is unconditional.
      It came off in the same change that deployed share.prepeat.app, which is
           what the item asked for - the failure mode was forgetting it in either
           direction, and neither happened.
      **The trap this item existed to name is still true and still worth
           keeping:** a rule enforced in an app build is a rule that leaks from
           every old version ever shipped. That is why `create_recipe_share`
           decides what may be published, not the client.

- [x] **`eas-submit-ios.sh` reports the wrong build as proof – FIXED 2026-08-19**
      It happened again on the very next release, exactly as predicted here:
           submitting build 25 printed **"Build 24 is VALID on TestFlight"**
           seconds later. Same cause as the 2026-08-17 sighting with builds 20
           and 17 - a build Apple has not finished ingesting is not in the list
           at all, so the PREVIOUS build is the newest and satisfies the check.
      Fixed as this item proposed: `asc-build-state.mjs --expect <build>` waits
           for that number and nothing else, and `eas-submit-ios.sh` reads the
           number out of the submit's own output and passes it. If the number
           cannot be read the script SAYS the check is about the newest build
           rather than silently checking the wrong thing. Both paths tested
           against the live API: `--expect 25` exits 0, `--expect 999` exits 1.
      Build 25 was then confirmed VALID by polling for 25 specifically.

- [x] **The Settings switch row agrees across every frame – DONE 2026-08-20**
      Verified document-wide rather than on the two frames the item named:
           **ten Settings frames** carry that row - edit kitchen, delete
           kitchen, edit profile, leave kitchen, delete profile, change kitchen
           1 and 2, default 3 and 4, and invite - and **every one now reads
           "Screen stays on" + "Only while you are on a recipe"**. Zero matches
           anywhere for the old "while cooking" wording.
      How it went: Claude fixed the label on `709:7582`; Thomas added the hint
           row, restructuring the variant (the old node id no longer exists, so
           it was rebuilt rather than edited). The recipe-screen frames were
           updated separately the same evening.
      ⚠️ **THE ITEM NAMED TWO FRAMES AND THE FILE HAD TEN.** It said "one row,
           copied across", scoped to `default 3` vs `default 4`. Searching for
           the STRING rather than trusting the ids is what showed the real
           spread - the same lesson as the ⋯ button, where two named ids turned
           out to be five matches across two pages.
...
- [x] `RETIRED 2026-08-17, Thomas's call.` **Leaving stays where it is –
      Settings → your name → Edit profile.** The switcher does not get a leave
      affordance.
      **Let people leave a kitchen from the switcher**
      ⚠️ **NOTHING IS BROKEN BY THIS.** Leaving works today and always did; the
           item was about WHERE it lives, not whether it exists. Do not read
           this retirement as "you cannot leave a kitchen".
      Why retired: the trigger was Thomas looking in the wrong place ONCE, on
           2026-08-07 – and on a kitchen he had just created solo, where the
           option is correctly absent anyway. So the evidence was one miss under
           conditions that would have hidden it regardless.
      **AND THE GROUND HAS MOVED SINCE.** The item says "Kitchen tab", which
           stopped existing on 2026-08-13 when the switcher moved INTO Settings.
           The two surfaces are now one screen apart rather than in different
           places, which was most of the argument for moving it.
      ⚠️ **The reservation that was written here still holds, and is worth
           keeping for anyone who reopens it:** leaving is irreversible-ish –
           you get a copy of the recipes, not the shared kitchen back – so it
           should never sit one tap from a routine switch. That was the argument
           AGAINST the switcher all along.
      Reopen if: someone other than Thomas cannot find how to leave. One
           person looking in the wrong place once is not that.
...
- [x] `RETIRED 2026-08-17, Thomas's call.` **The invite code stays as it is.**
      **Show the invite code as a filled brand chip**
      ⚠️ **THERE WAS NEVER ANYTHING WRONG TO REPAIR** – which is the whole
           reason retiring costs nothing. The accessibility defect this grew out
           of was fixed on 2026-08-02 with `text/default` at 9.12:1, and both
           invite surfaces already agree. The chip was an enhancement on top of
           a solved problem: white on `#378112` clears AA at 4.87:1 and would
           make the code look more like a thing to be copied.
      Why retired: its own last line was *"only worth doing if Thomas wants to
           design it"*, and he does not. An item whose entire condition is
           "if the designer feels like it" is answered the moment he says no.
...
- [ ] **Re-seed the reviewer's demo account before every submission**
      ✅ **The mailbox check is CLOSED (Thomas, 2026-08-20): *"It's set up and
           don't change. So you don't need me for that task."*** The
           `appreview@sebell.dk` sign-in is stable, so confirming a one-time
           code is no longer a step before each release. **Nothing on this item
           waits on Thomas any more** – the seeding is one command and the
           pre-flight blocks without it.
           The one thing that would break silently, recorded once and not
           re-argued: if that mailbox ever does change, nothing here notices,
           and the symptom is a rejected review rather than a warning. Cheap to
           re-check if a review is ever refused on sign-in.
      ...
      **The App Review Information** in App Store Connect (username, mailbox
           password, §3 notes with the webmail URL) is set and stable for the
           same reason. Worth a glance only if the account details ever move.
...
- **2026-08-20 – RECIPE TAGS ARE SHARED, ANYONE MAY EDIT THEM, AND THEY DO NOT
  LEAVE THE HOUSEHOLD.** Thomas settled three of the five open questions on the
  tags idea in one line each: favourites stay their own thing and do not become
  a tag; tags belong to the household and so do not travel to another one when a
  recipe is shared; anybody in the household can edit or delete any tag. The tag
  filter goes on the plan screen's add-meal sheet as well as the recipes list.
  **The through-line, worth naming because it will settle the next question
  too:** a household is four people who talk to each other. Every one of these
  answers refuses a permission model. There is no owner, no per-person tag, no
  ...
  **AGREED THE SAME DAY (Thomas: *"I agree on your assessment on no. 1"*):
  ...
  - **The two examples are different KINDS of tag.** "Dessert" is a *course* –
    it partitions the cookbook, and it is roughly exclusive with "main".
    "Protein rich" is a *property* – it cuts across every course. Properties are
    what people combine, and combining properties only ever means AND.
  - **A family cookbook is SMALL, and that decides it.** With forty recipes, OR
    barely narrows anything – you were already looking at a browsable list.
    Narrowing to a handful is the only useful direction, and AND narrows while
    OR widens. The argument would flip on a library of thousands.
  - **The earlier objection was that a chip row cannot SHOW which operator it
    means. That is true and it does not matter**, because the result set shows
    it: tap two chips, watch thirty recipes become three, and the meaning is
    learned in one tap without a word of explanation.
  - **The one real cost is zero results**, which is where AND stops teaching and
    starts looking broken. So the empty state is not a detail – it has to name
    both tags ("No recipe is both protein rich and dessert") and offer a one-tap
