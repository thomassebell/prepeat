...
- [ ] **⚠️ Generate the share preview card as a PNG (`og:image`) – REQUIRED, not polish**
      ...
      Watch: this is the growth mechanic's weakest link, and it is the common
           case rather than the edge case. Measure the install rate before AND
           after, or the work cannot be judged.
      Alternatives worth weighing first, all cheaper: **ask for a photo at share
           time** ("add a photo and this looks like yours") which produces
           content unambiguously ours; per-field provenance so a replaced photo
           can travel; or attribution ("recipe from foodsite.com") instead of
           content.

      (2026-08-17, after measuring the ratio.)

- [ ] **⚠️ Every SECURITY DEFINER function is executable by `anon`**
      Needs: nothing – but it wants the household-boundary test as a safety net,
           which is why it was not swept up mid-feature.
      Why: Postgres grants EXECUTE to PUBLIC on every new function, and Supabase
           ALSO grants directly to `anon` by default privilege. So
           `delete_profile`, `delete_household`, `join_household_with_code`,
           `leave_household`, `rotate_invite_code`, `is_household_member` and
           the rest are all callable with the public anon key today.
      Exploitable? Mostly no – each one gates on `is_household_member()` or
           `auth.uid()`, which is null for anon, so the call raises instead of
           doing anything. **The one that matters is
           `join_household_with_code`:** an anon caller can reach it and burn the
           GLOBAL invite-guess budget from 0033, which is a denial-of-service on
           joining, not a way in.
      Careful: `is_household_member` is called from inside RLS policies, so
           revoking it changes behaviour for every policy that uses it. Do it
           with `supabase/tests/household-boundary.sql` running before and after.
      Pattern to copy: migrations 0036 and 0037 did exactly this for the three
           sharing functions – revoke from `public` AND from `anon` by name,
           then check the LIVE ACL, because a local `db:reset` cannot prove a
           grant change.

      (Found 2026-08-18 while pushing 0035, by checking the live grants instead
      of trusting the grant statement.)

- [ ] **Copy-on-leave loses ingredient section headers**
      Why: `leave_household()` (migration 0015) copies recipe_ingredients
           without `is_section`, which arrived later in 0031. So a section
           header like "DOUGH" is copied as an ordinary ingredient – and becomes
           a tickable line in the leaver's cookbook, which is the exact thing
           sections exist to prevent.
      Size: one column in one INSERT ... SELECT.
      Note: `save_shared_recipe()` (0035) copies it correctly; this is the older
           twin that was never updated.

      (Found 2026-08-18 while writing 0035, by diffing the live column list
      against the one 0015 copies.)
