...
- [ ] `2.10` **⭐ Show IN THE APP that a meal can go on the plan without a recipe**
      Needs: Thomas – a frame for a line on the EMPTY PLAN saying a meal need
      ...
      Watch when the frame lands: the tabs are equal halves with no
           `numberOfLines`, so a label too long WRAPS to two lines rather than
           truncating. "Anything else" measures ~90pt in ~160pt, so it fits –
           but any future label needs the same check.
