...
- **The other Settings frame is missing the hint row** – one row added to a variant. The label half is done.
  <sub>Someday – not committed</sub>
...
- [ ] **The other Settings frame is missing the hint row**
      Needs: Thomas – one row added to a variant. The label half is done.
      ✅ **Label fixed 2026-08-20:** `709:7582` (`default 4`, the three-People
           variant) read "Screen on while cooking"; it now reads **"Screen stays
           on"**, matching `684:3866` and the app. Nothing in the file carries
           the old wording any more - checked document-wide, zero matches.
      ⬜ **Still differs:** `684:3866` has TWO text layers, the label **and** the
           hint *"Only while you are on a recipe"*. `709:7582` has only the
           label. **Adding a row is a structural change to the design, not a
           copy-paste**, so it was left for Thomas rather than invented.
      ⚠️ **Thomas believed this was already updated (2026-08-20) and it was
           not** - the frame still held the old text when checked. Worth the
           thirty seconds it took to look: the item would otherwise have been
           ticked with the disagreement still in the file, which is precisely
           the failure it was raised to prevent.
