...
- **⭐ Share a recipe with someone – FIRST ITEM IN v1.1** – DESKTOP frames for the share site, a 404 frame for it, and a ruling on the ⋯ button. The PHONE designs landed 2026-08-18 (Messages 725:7752, share site 726:8992, in-app receive 725:8341).
  <sub>Next – v1.1</sub>
...
- **⭐ FOR THE MORNING: publish the privacy policy, then submit 1.1.0** – three reviews, in this order. Everything else is done.
  <sub>Someday – not committed</sub>
- **⚠️ The submitted "What's New" does not mention expiry or Stop sharing** – decide whether to edit it, and it is time-limited.
  <sub>Someday – not committed</sub>
...
- [ ] **⭐ Share a recipe with someone – FIRST ITEM IN v1.1**
      Needs: Thomas – DESKTOP frames for the share site, a 404 frame for it, and
           a ruling on the ⋯ button. The PHONE designs landed 2026-08-18
           (Messages 725:7752, share site 726:8992, in-app receive 725:8341).
...
- [ ] **⭐ FOR THE MORNING: publish the privacy policy, then submit 1.1.0**
      Needs: Thomas – three reviews, in this order. Everything else is done.
      1. **Read and publish the privacy policy.** `prepeat-web/privacy.html` has
         a new "Sharing a recipe outside your household" section **prepared but
         NOT committed** – committing there auto-deploys to the live site Apple
         links to, so it needs your eyes first. `docs/privacy-policy.md` (same
         wording) is already committed, so nothing is lost.
         Publish with: `cd ~/Documents/Claude/Projects/prepeat-web && git add
         privacy.html && git commit -m "..."` – the hook pushes and GitHub Pages
         deploys.
      2. **Review the What's New, then submit 1.1.0.** Already created in App
         Store Connect: **1.1.0, PREPARE_FOR_SUBMISSION, MANUAL release, build
         24 attached, release notes set and read back from Apple.** Claude did
         NOT submit – that reaches real users and is Thomas's call.
      3. **Proof-read the Danish** – the spreadsheet Claude sent: 281 unique
         strings, duplicates removed, 11 flagged, no placeholder mismatches.
      ⚠️ **A gap the policy exposed:** there is no way for a user to turn a share
         link off themselves, so the policy has to say "write to us and we will
         do it". Honest, but weak as a GDPR withdrawal route – it makes the
         in-app revoke more than a nicety.
...
- [ ] **⚠️ The submitted "What's New" does not mention expiry or Stop sharing**
      Needs: Thomas – decide whether to edit it, and it is time-limited.
      The App Store release notes attached to 1.1.0 were written for **build 24**
           and never revised. 1125 characters covering sharing, Danish, the
           screen staying on, imports and settings – all accurate, and silent on
           the two things build 25 actually added. **So users are told they can
           share, and not told that links lapse after 30 days or that they can
           be turned off.** The privacy policy, published the same day, says
           both.
      Why it matters more than a missing bullet: expiry makes something a user
           did stop working later. Finding that out from the policy rather than
           the release notes is a poor way to learn it, and "Stop sharing" is
           the answer to the question the feature raises.
      ⚠️ **TIME-LIMITED.** Metadata is normally editable while a version is
           WAITING_FOR_REVIEW and frozen once it is IN_REVIEW. Editing may also
           bounce the submission back, so the trade is: two better sentences
           against possibly re-queueing behind a 13-day review.
      The two missing lines are written and ready in
           [release-notes.md](release-notes.md) under 1.1.0, marked *(NOT in the
...
- [ ] **⚠️ UNGATE "Share recipe" when the share page ships**
      Needs: nothing – it is one `__DEV__` check to delete, in the same change
           that deploys the page.
      Why: the action exists in the recipe's ⋯ menu but only in dev builds,
           because the link resolves to nothing until the page is live. A share
           that hands someone a dead link is worse than no share, and a tester
           would send it to family.
